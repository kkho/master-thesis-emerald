...
import com.emerald.startup.setup.filemanager;
import java.io.File;
		...
		public void handleMessage(Message zMsg) {
			...
			if (msg.containsKey("close_install")) {
				...
				SharedPreferences prefs = PreferenceManager
						.getDefaultSharedPreferences(installer.this);
				String current = prefs.getString("CURRENT_SYSTEM",
						"no system installed");
				TextView tv = (TextView) findViewById(R.id.install_sys);
				tv.setText("Current   : " + current + "\n" + "Available : "
						+ CURRENT_INSTALL_SYSTEM);
		...
		if (zButton == findViewById(R.id.install_start)) {
			...
			Thread tt = new Thread() {
				public void run() {
					...
					msg.getData()
							.putString("info", "Starting System install..");
					...
					try {
						...
						// Working directory
						File worker = new File(tmp, "WORK_"
						...
						filemanager.extractAsset(installer.this, "busybox.mp3",
								busytar);
						...
						// Process pp =
						// Runtime.getRuntime().exec("chmod 770 "+busytar.getPath());
						Process pp = Runtime.getRuntime().exec(
								"chmod 770 " + busytar.getPath(), env, home);
						pp.waitFor();

						msg = new Message();
						msg.getData().putString("info",
								"Preparing " + CURRENT_INSTALL_SYSTEM + " ..");
						mInstallHandler.sendMessage(msg);

						File systar = new File(worker, "system.tar.gz");
						filemanager.extractAsset(installer.this,
								CURRENT_INSTALL_ASSETFILE, systar);

						// Now start
						msg = new Message();
						msg.getData()
								.putString("info", "Removing Old System..");
						mInstallHandler.sendMessage(msg);
						...
						pp = Runtime.getRuntime().exec(
								busytar.getPath() + " tar -C " + home.getPath()
										+ " -xzf " + systar.getPath(), env,
								home);
						pp.waitFor();
						...
						msg = new Message();
						msg.getData().putString("info",
								"Installing BusyBox Apps..");
						mInstallHandler.sendMessage(msg);
						

						msg = new Message();
						msg.getData().putString("info",
								"Installing new system.. can take a minute");
						mInstallHandler.sendMessage(msg);
						...
						// pp =
						// Runtime.getRuntime().exec(busybox.getPath()+" --install -s "+bbindir.getPath());
						pp = Runtime.getRuntime().exec(
								busybox.getPath() + " --install -s "
										+ bbindir.getPath(), env, home);
						pp.waitFor();
						...
						if (!bashrcu.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+bashrc.getPath()+" "+bashrcu.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -f "
											+ bashrc.getPath() + " "
											+ bashrcu.getPath(), env, home);
							pp.waitFor();
						}
						...
						if (!nanorcu.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+nanorc.getPath()+" "+nanorcu.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -f "
											+ nanorc.getPath() + " "
											+ nanorcu.getPath(), env, home);
							pp.waitFor();
						}
						...
						if (!tmuxrcu.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+tmuxrc.getPath()+" "+tmuxrcu.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -f "
											+ tmuxrc.getPath() + " "
											+ tmuxrcu.getPath(), env, home);
							pp.waitFor();
						}
						...
						if (!mcini.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+ini.getPath()+" "+mcini.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -f "
											+ ini.getPath() + " "
											+ mcini.getPath(), env, home);
							pp.waitFor();
						}
						...
						// pp =
						// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+inputrc.getPath()+" "+inputrcu.getPath());
						pp = Runtime.getRuntime().exec(
								busytar.getPath() + " cp -f "
										+ inputrc.getPath() + " "
										+ inputrcu.getPath(), env, home);
						pp.waitFor();
						...
						if (!vimrcu.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -f "+vimrc.getPath()+" "+vimrcu.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -f "
											+ vimrc.getPath() + " "
											+ vimrcu.getPath(), env, home);
							pp.waitFor();
						}
						...
						if (!vimhu.exists() || mOverwriteAll) {
							// pp =
							// Runtime.getRuntime().exec(busytar.getPath()+" cp -rf "+vimh.getPath()+" "+vimhu.getPath());
							pp = Runtime.getRuntime().exec(
									busytar.getPath() + " cp -rf "
											+ vimh.getPath() + " "
											+ vimhu.getPath(), env, home);
							pp.waitFor();
						}
						...
						String func = busytar.getPath() + " ln -s "
								+ sdcard.getPath() + " " + lnsdcard.getPath();
						...
						editor.putString("CURRENT_SYSTEM",
								CURRENT_INSTALL_SYSTEM);
						editor.putInt("CURRENT_SYSTEM_NUM",
								CURRENT_INSTALL_SYSTEM_NUM);
						editor.commit();

					} catch (Exception iOException) {
						Log.v("SpartacusRex", "INSTALL SYSTEM EXCEPTION : "
								+ iOException);

						msg = new Message();
						msg.getData()
								.putString("error", iOException.toString());
						mInstallHandler.sendMessage(msg);
